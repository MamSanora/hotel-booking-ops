                            ...
                            <div class="flex justify-end gap-2">
                                @if($booking->booking_status === 'pending')
                                    <form action="{{ route('admin.bookings.approve', $booking->id) }}" method="POST">
                                        @csrf @method('PATCH')
                                        <button type="submit" class="bg-green-100 hover:bg-green-200 text-green-700 px-3 py-1.5 rounded-md text-sm font-semibold transition-colors" title="Approve">
                                ...
                                @if(in_array($booking->booking_status, ['pending', 'booked']))
                                    <form action="{{ route('admin.bookings.cancel', $booking->id) }}" method="POST">
                                        @csrf @method('PATCH')
                                        <button type="submit" onclick="return confirm('Cancel this booking?')" class="bg-orange-100 hover:bg-orange-200 text-orange-700 px-3 py-1.5 rounded-md text-sm font-semibold transition-colors" title="Cancel">
                                            <i class="bi bi-x-lg"></i>
                                        </button>
                                    </form>
                                @endif
                                <form action="{{ route('admin.bookings.destroy', $booking->id) }}" method="POST">
                                    @csrf @method('DELETE')
                                    <button type="submit" onclick="return confirm('Permanently delete this booking?')" class="bg-red-100 hover:bg-red-200 text-red-700 px-3 py-1.5 rounded-md text-sm font-semibold transition-colors" title="Delete">
