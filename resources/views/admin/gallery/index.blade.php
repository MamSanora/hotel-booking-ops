            ...
            @foreach($gallery as $item)
                <div class="group relative bg-white rounded-xl overflow-hidden shadow-[0_4px_16px_rgba(0,0,0,0.07)] hover:shadow-[0_8px_30px_rgba(0,0,0,0.13)] transition-all duration-300">
                    ...
                    <div class="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
                        <form action="{{ route('admin.gallery.destroy', $item) }}" method="POST" x-data @submit.prevent="$dispatch('open-confirm', { message: 'Delete this image?', action: (function(f) { return () => f.submit(); })($el) })">
                            @csrf @method('DELETE')
