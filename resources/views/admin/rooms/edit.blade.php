        ...
        <form action="{{ route('admin.rooms.update', $room) }}" method="POST" class="space-y-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <select name="room_type" required class="w-full px-4 py-3 rounded-xl border border-gray-200 focus:border-hotel-gold focus:ring-2 focus:ring-hotel-gold/20 transition-all text-[0.95rem]">
                        <option value="standard_twin"   {{ old('room_type', $room->room_type) === 'standard_twin'   ? 'selected' : '' }}>Standard Twin</option>
                        <option value="standard_double" {{ old('room_type', $room->room_type) === 'standard_double' ? 'selected' : '' }}>Standard Double</option>
                        <option value="deluxe_double"   {{ old('room_type', $room->room_type) === 'deluxe_double'   ? 'selected' : '' }}>Deluxe Double</option>
                        <option value="family_room"     {{ old('room_type', $room->room_type) === 'family_room'     ? 'selected' : '' }}>Family Room</option>
                ...
                <div>
                    ...
                    <input type="number" name="capacity" value="{{ old('capacity', $room->capacity) }}" required min="1" max="10"
                           class="w-full px-4 py-3 rounded-xl border border-gray-200 focus:border-hotel-gold focus:ring-2 focus:ring-hotel-gold/20 transition-all text-[0.95rem]">
                ...
                <div>
                    ...
                    <div class="relative">
                        ...
                        <input type="number" name="price_per_night" value="{{ old('price_per_night', $room->price_per_night) }}" required step="0.01" min="0"
                ...
                {{-- Status --}}
                <div>
                    <label class="block text-[0.85rem] font-semibold text-gray-700 uppercase tracking-wider mb-2">Status</label>
                    <select name="current_status" required class="w-full px-4 py-3 rounded-xl border border-gray-200 focus:border-hotel-gold focus:ring-2 focus:ring-hotel-gold/20 transition-all text-[0.95rem]">
                        <option value="available"   {{ old('current_status', $room->current_status) === 'available'   ? 'selected' : '' }}>Available</option>
                        <option value="occupied"    {{ old('current_status', $room->current_status) === 'occupied'    ? 'selected' : '' }}>Occupied</option>
                        <option value="maintenance" {{ old('current_status', $room->current_status) === 'maintenance' ? 'selected' : '' }}>Maintenance</option>
                    </select>
            ...
            <div>
                ...
                <textarea name="description" rows="4"
                          class="w-full px-4 py-3 rounded-xl border border-gray-200 focus:border-hotel-gold focus:ring-2 focus:ring-hotel-gold/20 transition-all text-[0.95rem]">{{ old('description', $room->description) }}</textarea>
            </div>
