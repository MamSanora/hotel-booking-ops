...
<head>
    ...
    <title>Verify Phone — Dara Meas Hotel</title>
    <meta name="description" content="Enter the verification code sent to your phone number.">
    ...
    <div class="bg-white rounded-2xl shadow-[0_4px_30px_rgba(0,0,0,0.08)] border border-gray-100 p-8 md:p-10">
        ...
        <div class="text-center mb-8">
            <div class="w-16 h-16 rounded-full bg-hotel-gold/10 flex items-center justify-center mx-auto mb-4">
                <i class="bi bi-phone text-3xl text-hotel-gold"></i>
            </div>
            <h1 class="font-[Playfair_Display] text-2xl font-bold text-hotel-dark mb-2">Check your phone</h1>
            <p class="text-sm text-gray-500 leading-relaxed">
                We've sent a 6-digit verification code to<br>
                <span class="font-semibold text-hotel-dark">
                    {{-- Mask the phone: show first 4 and last 3 digits --}}
                    @php
                        $masked = preg_replace('/(\+?\d{3,4})\d+(\d{3})/', '$1 *** *** $2', $phone ?? '');
                    @endphp
                    {{ $masked ?: $phone }}
                </span>
            </p>
        ...
        {{-- Success flash --}}
        @if(session('success'))
            <div class="bg-green-50 border border-green-200 text-green-800 rounded-xl px-4 py-3 text-sm mb-5 flex items-center gap-2">
                <i class="bi bi-check-circle-fill text-green-500"></i> {{ session('success') }}
            </div>
        @endif

        {{-- OTP Form --}}
        <form method="POST" action="{{ route('guest.verify-phone.submit') }}" id="otp-form" class="space-y-6">
            @csrf
            ...
            <button type="submit"
                ...
                <i class="bi bi-shield-check"></i> Verify Phone Number
            </button>
        </form>
        ...
        <div class="mt-6 text-center">
            ...
            <form method="POST" action="{{ route('guest.verify-phone.resend') }}" class="inline">
                @csrf
                <button type="submit" class="text-sm text-hotel-gold hover:text-amber-600 font-semibold transition-colors mt-1">
        ...
        <div class="mt-4 text-center">
            <a href="{{ route('guest.register') }}" class="text-xs text-gray-400 hover:text-gray-600 transition-colors">
                <i class="bi bi-arrow-left mr-1"></i> Back to register
            </a>
        </div>
