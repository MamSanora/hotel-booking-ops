<div class="flex flex-col lg:flex-row items-stretch gap-6 lg:gap-8">
    ...
    <div class="w-full lg:w-[55%] flex flex-col">
        <div class="bg-white rounded-2xl shadow-[0_4px_20px_rgba(0,0,0,0.06)] border border-gray-100 flex flex-col lg:sticky lg:top-[4.5rem]">
    ...
    <div class="w-full lg:w-[45%] flex flex-col">
        <form wire:key="booking-form" action="{{ route('reception.manual-booking.store') }}" method="POST" class="bg-white rounded-2xl shadow-[0_4px_20px_rgba(0,0,0,0.06)] border border-gray-100 flex flex-col lg:sticky lg:top-[4.5rem] lg:h-[calc(100vh-6rem)] overflow-hidden">
            @csrf
                ...
                <div class="space-y-6 border-t border-gray-100 pt-6">
                    <div>
                        ...
                        <div>
                            ...
                            <div class="mt-6" wire:key="payment-reference-block">
                    ...
                    {{-- Price Summary --}}
                    <div wire:key="price-summary-block" class="bg-hotel-light rounded-xl p-5 border border-[#e8e0d0]">
                        <div class="flex justify-between text-[0.9rem] text-gray-600 mb-2">
                            <span>Rate per night</span>
                            <span class="font-medium text-gray-800" x-text="'$' + Number($wire.pricePerNight).toFixed(2)">${{ number_format($pricePerNight, 2) }}</span>
                        </div>
                        <div class="flex justify-between text-[0.9rem] text-gray-600 mb-2">
                            <span>Number of nights</span>
                            <span class="font-medium text-gray-800" x-text="$wire.nights">{{ $nights }}</span>
                        </div>
                        <div class="flex justify-between items-center text-[1.05rem] font-bold text-hotel-dark border-t border-[#e0d8cc] pt-3 mt-1 mb-3">
                            ...
                            <span class="text-hotel-gold text-xl" x-text="'$' + Number($wire.totalPrice).toFixed(2)">${{ number_format($totalPrice, 2) }}</span>
                        </div>
                        
                        <div class="flex justify-between items-center text-[0.95rem] font-bold text-hotel-dark pt-1">
                            <span>Amount Due Now (<span x-text="$wire.paymentTier">{{ $paymentTier }}</span>%)</span>
                            <span class="text-green-700" x-text="'$' + Number($wire.amountDue).toFixed(2)">${{ number_format($amountDue, 2) }}</span>
                        </div>
